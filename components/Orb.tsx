...
export default function Orb({
  ...
  useEffect(() => {
    ...
    // Pause RAF loop when tab is hidden to prevent main-thread stalling
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let meshRef: any = null
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let programRef: any = null
    let lastTime = 0
    let currentRot = 0
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let hexToVec3Fn: (color: string) => any

    function update(t: number) {
      if (disposed || paused) return
      rafId = requestAnimationFrame(update)
      if (!rendererRef || !meshRef || !programRef) return
      ...
      programRef.uniforms.bgColor.value = hexToVec3Fn(backgroundColor)

      const effectiveHover = forceHoverState ? 1 : targetHover
      programRef.uniforms.hover.value +=
        (effectiveHover - programRef.uniforms.hover.value) * 0.1
      ...
      rendererRef.render({ scene: meshRef })
    }
    ...
    import("ogl").then(({ Renderer, Triangle, Program, Mesh, Vec3 }) => {
      ...
      const frag = /* glsl */ `
        ...
        const vec3 baseColor1 = vec3(0.486, 0.227, 0.929);
        const vec3 baseColor2 = vec3(0.125, 0.439, 0.910);
        const vec3 baseColor3 = vec3(0.080, 0.060, 0.340);
      ...
      const program = new Program(gl as any, {
        ...
        uniforms: {
          ...
          bgColor: { value: hexToVec3Fn(backgroundColor) },
        },
      })
